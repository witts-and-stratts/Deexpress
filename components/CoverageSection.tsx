...
export function CoverageSection({ ctaHref = '/destinations' }: { ctaHref?: string }) {
  ...
  const region = active ? t.regions[active] : null
  ...
  return (
    <div className="grid items-stretch gap-8 lg:grid-cols-12 lg:gap-14">
      <div className="relative min-h-[420px] overflow-hidden rounded-3xl border border-white/6 bg-navy-950 p-5 lg:col-span-7 sm:p-9">
        <DottedMap active={active} onSelect={setActive} />
      ...
      <div className="flex flex-col gap-5 lg:col-span-5">
        {region ? (
          <div key={active} className="flex flex-1 flex-col gap-3 rounded-3xl border border-slate-200 bg-white p-7 transition-colors duration-300 hover:border-royal/40 sm:p-9">
          ...
          <div className="flex flex-1 flex-col justify-center gap-3 rounded-3xl border border-slate-200 bg-white p-7 sm:p-9">
            <span className="text-[0.72rem] font-bold tracking-[0.12em] uppercase text-orange">{t.home.mapHint}</span>
            <h3 className="font-display text-2xl font-semibold">{t.home.coverageTitle}</h3>
            <p className="leading-relaxed text-slate-500">{t.home.coverageSub}</p>
          </div>
        )}

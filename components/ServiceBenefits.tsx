...
import { SERVICE_ENHANCEMENTS } from '@/lib/service-content'
import type { ServiceSlug } from '@/lib/site'

export function ServiceBenefits({ service }: { service: ServiceSlug }) {
  const { lang } = useLang()
  const benefits = SERVICE_ENHANCEMENTS[lang][service]

  return (
    <section className='service-benefits' aria-labelledby={`${service}-benefits-title`}>
      <Reveal>
        <h2 id={`${service}-benefits-title`}>{benefits.optionsTitle}</h2>
      </Reveal>
      <div className='service-benefits__grid'>
        {benefits.options.map(({ title, text }, index) => (
